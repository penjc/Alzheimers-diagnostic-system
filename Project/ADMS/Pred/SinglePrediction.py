...
def Prediction(username, MRI_Name):
    ...
    n_test = X.shape[0]
    print(n_test)
    model_name1 = r'D:\AD_Prediction\Project\ADMS\Model\Unet_model.100.hdf5'  # 加载预测的模型,可变数据
    model_name2 = r'D:\AD_Prediction\Project\ADMS\Model\Dense_unet_model.100.hdf5'
    ...
    save_pred_folder = r'D:\AD_Prediction\Project\ADMS\static\requiredimages\admin\Pred/'
    # 测试用
    ...
    for i in range(n_test):
        ...
        if ifsave:
            ...
            img_affine = tImg.affine
            #nib.save(tImg, save_pred_folder + df[:-4] + '.nii')
            nib.save(nib.Nifti1Image(2 * pr.astype('float'), img_affine), save_pred_folder + df[:-4] + '.nii')  # 存储具体的预测结果图，可变数据
