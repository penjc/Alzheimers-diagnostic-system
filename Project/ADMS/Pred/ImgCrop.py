...
def image_crop(username, MRI_Name):
    img_name =  r'D:\AD_Prediction\Project\ADMS\static\requiredimages\user'
    basename =  username + "/upload/" + MRI_Name
    img_name = os.path.join(img_name, basename)

   #img_name = r'D:\AD_Prediction\Project\ADMS\static\requiredimages\user\1111112\upload\1.png'

    label_name = r'D:\AD_Prediction\Project\ADMS\static\requiredimages\label'
    base_name = MRI_Name
    label_name = os.path.join(label_name, base_name)
    # 测试用
    ...
    base = r'D:\AD_Prediction\Project\ADMS\static\requiredimages\admin\Croped/'
    nib.save(nib.Nifti1Image(image_block, timg.affine), base + img)
    print('croped！')
