...
def login():
    ...
    if request.method == 'POST':
        ...
        print(password)
        print(type)
        db = pymysql.connect(host="127.0.0.1", port=3306, user="root", password="root", db="adms")
        if type == "普通用户":
            cursor = db.cursor()
            ...
            try:
                ...
                if len(results) == 1:
                    ...
                    # select_experts = "SELECT e_id, e_name, e_sex, e_birth FROM expert"
                    # cursor.execute(select_experts)
                    # expert_data = cursor.fetchall()
                    #
                    # # Insert expert data into the visibility table for the logged-in user
                    # insert_visibility = "INSERT INTO visibility (user_id, expert_id, expert_name, expert_sex, expert_birth) VALUES (%s, %s, %s, %s, %s)"
                    # for expert in expert_data:
                    #     cursor.execute(insert_visibility, (username, expert[0], expert[1], expert[2], expert[3]))
                    #
                    # db.commit()
                    # db.close()
    ...
    if request.method == 'POST':
        ...
        if data['flag'] == 0:  # 获得状态码0则说明该用户已经被注册
            ...
            try:
                ...
                insert_visibility = "INSERT INTO visibility (user_id, expert_id, expert_name, expert_sex, expert_birth) VALUES (%s, %s, %s, %s, %s)"
                for expert in expert_data:
                    cursor.execute(insert_visibility, (UserID, expert[0], expert[1], expert[2], expert[3]))
...
def UserDApply():
    if 'username' in session:
        ...
        expert = "select * from visibility where user_id='" + username + "'"

        pred = "select * from pred where p_uid='" + username + "'"

        try:
            # 执行sql语句
            cursor.execute(expert)
            results = cursor.fetchall()
            cursor.execute(mri)
                ...
                if mri_re[0]:  # 如果结果不为空的话，就获得第一条数据
                    if mri_re[0][0]:  # 如果第一条数据不为空的话，就获得元组中的第一个值
                        ...
                        if da_re:
                            for i in da_re:
                                if i[4] == "申请中":
                                    ...
                                    i = i + (action, 'btn-success', 'submit', '审核')
                                elif i[4] == "申请完毕，请预测":
                                    ...
                                    pred = '<form method="post" action=' + action1 + '>' \
                                                                                     ...
                                                                                     '</form>'
                                    i = i + ("#", 'btn-inverse', 'button', '已审核', pred)
                                elif i[4] == "预测完毕":
                        ...
                        return render_template('user/UserDApply.html', username=session['username'],
                                               MRI_Name=mri_re[0][2], dapply=status_apply, expert=results,
                                               predict=predict, num=num)
                    else:
                        ...
                        db.commit()
                        return render_template('user/UserDApply.html', username=session['username'],
                                               MRI_Name="未获得MRI影像名称", dapply=da_re, expert=results)
...
@app.route('/updateVisibility', methods=['POST'])
def updateVisibility():
    if 'username' in session:
        try:
            expertName = request.form['expertName']
            visibility = int(request.form['visibility'])

            db = pymysql.connect(host="127.0.0.1", port=3306, user="root", password="root", db="adms")
            cursor = db.cursor()

            # Update the visibility in the visibility table
            update_query = "UPDATE visibility SET visible = %s WHERE expert_name = %s AND user_id = %s"
            cursor.execute(update_query, (visibility, expertName, session['username']))
            db.commit()
            ...
            traceback.print_exc()
            db.rollback()
            return "An error occurred while updating visibility."
    else:
...
def UserResu():
    if 'username' in session:
        ...
        try:
            ...
            revi = "SELECT * FROM user where u_id='" + username + "'"
            cursor.execute(revi)
            ...
            if results:
                print(results)
                msg = "(诊断结果信息如下：)"
                return render_template('user/UserResu.html', username=session['username'], resu=results, review=review,
                                       msg1=msg)
            else:
                msg = "(未获取到诊断结果信息)"
            ...
            if ja_re:
                for i in ja_re:
                    ...
                    if i[6] == "申请中":
                        ...
                        action_1 = "/" + i[1] + "/4"  # 这一步是获得查看专家个人履历的URL
                        i = i + (
                        "/JApplyAllow", 'btn-inverse', 'button', '已批准', action_1, 'btn-success', 'submit', '查看')
                    status_apply += ((i),)
                print(status_apply)
...
def ExpertDApply():
    if 'username' in session:
        ...
        WHERE EXISTS(
            SELECT 1
        FROM visibility v
        WHERE v.user_id = u.u_id
        AND v.expert_id = '{}' 
        AND v.visible = 1
        )
        """.format(username)

        # dapply = "SELECT * FROM dapplication d INNER JOIN visibility v ON d.da_uid = v.user_id WHERE v.expert_id = '{}' AND v.visible = 1".format(username)
        dapply = """
            SELECT * FROM dapplication d
            WHERE EXISTS (
                SELECT 1 FROM visibility v
                WHERE d.da_uid = v.user_id
                AND v.expert_id = '{}'
                AND v.visible = 1
            )
            """.format(username)
            ...
            if da_re:
                for i in da_re:
                    if i[4] == "申请中":
                        ...
                        file = i[2]
                        pred = '<button class="btn-success"><a href=' + url + ' download=' + file + '>下载预测文件</a></button>'
...
@app.route('/update_review_time', methods=['POST'])
def update_review_time():
    ...
    # 构建更新语句
    update_query = "UPDATE user SET review = %s WHERE u_id = %s"

    # 执行更新操作
    cursor.execute(update_query, (review_time, user_id))

    db.commit()
    db.close()

    return redirect(url_for('ExpertDApply'))
    ...
    if 'username' in session:
        ...
        try:
            ...
            if results:
                ...
                print(results)
                msg = "(诊断结果信息如下：)"
                return render_template('expert/ExpertGetMRI.html', username=session['username'], resu=results_r,
                                       msg1=msg)
            else:
                msg = "(未获取到诊断结果信息)"
...
def UserUpload():
    ...
    if request.method == 'POST':
        try:
            ...
            dirlabel = 'static/requiredimages/label/'

            if os.path.exists(dir + '/upload'):
                # 同名文件已存在，进行处理
                # 可以给新文件重命名，例如在文件名后面加上时间戳
                import time
                # timestamp = str(int(time.time()))
                new_filename = filename  # + '_' + timestamp
                storageimage(dir + '/upload/' + new_filename, content)
                storageimage(dirlabel + new_filename, content)
                data = {'filename': new_filename}
                data = json.dumps(data)
                return data
            else:
                os.makedirs(dir + '/upload')
                storageimage(dir + '/upload/' + filename, content)
                storageimage(dirlabel + filename, content)
            ...
            if rmname_r:
                ...
                try:
                    ...
                    if mri_r:
                        ...
                        r_hipvolu = get_hipvlousum(username=r_eid,
                                                   MRI_Name=r_mname)  # 这里是通过MRI影像名称来获得该MRI影像，并计算出其海马体大小。5000为一个临时值
                        r_hipvolu = str(r_hipvolu)
                        # url = "D:\AD_Prediction\MRI.nii"
                        #
                        # mriurl = "UPDATE resu set r_path='" + url + "'where r_mname='" + r_mname + "'"
                        # cursor.execute(mriurl)
                        # 提交到数据库执行
                        # db.commit()
            ...
            if mri_re:
                if mri_re[0]:
                    if mri_re[0][0]:
                        ...
                        db.commit()
                        return render_template('user/UserDApply.html', username=session['username'],
                                               MRI_Name=mri_re[0][2], dapply=status_apply, msg=msg)
                    else:
                        cursor.execute(dapply)
                        da_re = cursor.fetchall()
                        db.commit()
    ...
    if request.method == 'POST':
        ...
        if number == 1:
            ...
            try:
                ...
                db.commit()
                msg = '成功批准！'
                return redirect(url_for('UserDApply'))
            except:
                # 抛出错误信息
                traceback.print_exc()
                # 如果发生错误则回滚
            ...
            try:
                ...
                if japply_r:
                    db.commit()
                    print("查看成功！")
                    return render_template('admin/AdminExpertResume.html', username=username, expertid=expertid,
                                           japply=japply_r)
                else:
                    db.commit()
                    print("暂无该专家用户的个人履历")
...
def AdminPred(userid, MRIName):
    ...
    if request.method == 'POST':
        ...
        if duration:
            ...
            if 4000 < r_hipvolu < 5000:
                classfy = "健康"
            elif 4000 >= r_hipvolu >= 3000:
                classfy = "轻度认知障碍"
            elif r_hipvolu < 3000:
                classfy = "阿尔兹海默症"
            ...
            pred_insert = "INSERT INTO pred(p_uid, p_mname, p_ptime, p_utime, p_hipvolu) VALUES " \
                          ...
                          + "','" + r_hipvolu + "')"
            predupdate = "UPDATE dapplication set pred='" + r_hipvolu + "'where da_mname='" + MRI_Name + "'"
            classfyupdate = "UPDATE dapplication set predclass='" + classfy + "'where da_mname='" + MRI_Name + "'"
            try:
                # 执行sql语句
                ...
                # 提交到数据库执行
                db.commit()
                cursor.execute(predupdate)
                db.commit()
                cursor.execute(classfyupdate)
                db.commit()
                msg = '预测完毕！'
                return redirect(url_for('UserDApply'))
            except:
